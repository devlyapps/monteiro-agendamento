      ...
      <div class="client-card ${colorClass}" onclick="openLoyaltyModal('${clientNameSafe}','${String(c.phone||'').replace(/\D/g,'')}',${loyalty})">
        ...
        <div class="client-card-bottom">
          ...
          <div style="display:flex; gap:8px;">
            ...
            <button class="icon-btn small" onclick="event.stopPropagation(); openEditClientModal('${clientNameSafe}','${phone}')" title="Editar dados">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20h9M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>
            </button>
...
/* ===================== ADMIN · EDITAR DADOS DO CLIENTE ===================== */
let editClientOriginalPhone = null;
function openEditClientModal(name, phone){
  editClientOriginalPhone = phone;
  document.getElementById('editClientName').value = name || '';
  document.getElementById('editClientPhone').value = formatPhoneBR(phone);
  document.getElementById('editClientModalOverlay').classList.add('show');
}
function closeEditClientModalDirect(){
  document.getElementById('editClientModalOverlay').classList.remove('show');
}
function closeEditClientModal(e){
  if(e.target === document.getElementById('editClientModalOverlay')) closeEditClientModalDirect();
}
async function saveClientContact(){
  const name = (document.getElementById('editClientName').value || '').trim();
  const newPhone = document.getElementById('editClientPhone').value.replace(/\D/g,'');
  if(!name){ toast('Digite o nome do cliente'); return; }
  if(newPhone.length < 10){ toast('Digite um celular válido com DDD'); return; }
  toast('Salvando...');
  const res = await apiPost('updateClientContact', { phone: editClientOriginalPhone, newPhone, name });
  if(res.error){ toast('Erro: ' + res.error); return; }

  const idx = clientesCache.findIndex(c => String(c.phone).replace(/\D/g,'') === editClientOriginalPhone);
  if(idx !== -1){ clientesCache[idx].name = name; clientesCache[idx].phone = newPhone; }
  // Mantém a Agenda/Caixa do admin coerentes sem precisar recarregar a página,
  // já que o backend também atualizou o telefone nos agendamentos existentes.
  if(newPhone !== editClientOriginalPhone){
    bookings.forEach(b => { if(String(b.clientPhone||'').replace(/\D/g,'') === editClientOriginalPhone) b.clientPhone = newPhone; });
  }

  closeEditClientModalDirect();
